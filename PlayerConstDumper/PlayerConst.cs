...
namespace PlayerConstDumper
{
    ...
    public class PlayerConst
    {
        ...
        public void Write(Stream FS)
        {
            ...
            FS.WriteColor(SearchLightColor);
        ...
        public Dictionary<string, string> CreateDictionary()
        {
            ...
            foreach (var prop in GetType().GetProperties())
            {
                ...
                {
                    result.Add(name, c.ToArgb().ToString());
                }
            }
            return result;
        }
        ...
        public static PlayerConst FromFile(FileInfo file)
        {
            ...
            foreach (var prop in typeof(PlayerConst).GetProperties())
            {
                ...
                if (obj is float)
                {
                    var f = float.Parse(dict[prop.Name]);
                    prop.SetValue(res, f);
                } else if (obj is short)
                {
                    var s = short.Parse(dict[prop.Name]);
                    prop.SetValue(res, s);
                } else if (obj is ushort)
                {
                    var s = ushort.Parse(dict[prop.Name]);
                    prop.SetValue(res, s);
                } else if (obj is byte)
                {
                    var s = byte.Parse(dict[prop.Name]);
                    prop.SetValue(res, s);
                } else if (obj is int)
                {
                    var s = int.Parse(dict[prop.Name]);
                    prop.SetValue(res, s);
                } else if (obj is Color)
                {
                    var s = int.Parse(dict[prop.Name]);
                    var c = Color.FromArgb(s);
                    prop.SetValue(res, c);
                }
            }
            return res;
